...
Sources/
  PhotogrammetryCore/      ロジック本体。SwiftUI / AppKit を import しない
    ...
    Preprocess/            大量の写真の仕分け（sort。docs/design-preprocess-merge.md）
      ...
      OverlapGraph         実測の重なりと予算配分                ← 純ロジック
      ThresholdEstimator   分布 → 閾値（判別分析）               ← 純ロジック
      QualityFilter        寄与しない写真の除外                  ← 純ロジック
      PhotoGrouping        重なり（無ければ証拠の合算）→ グループ ← 純ロジック
...
- **画素の相関で「重なっているか」を判定しようとしないこと**（設計メモ §4.9.2）。
  グループ分けを実際の重なりで決める試み（フェーズ 2.6）は実装したうえで
  **実データで棄却した**。3 次元の場面を別の立ち位置から撮った 2 枚を揃える
  2 次元の変換は存在せず、Vision が返す平行移動／射影変換の後で画素を突き合わせ
  ても、重なりの有無とほとんど関係が無い。実写真 1424 枚で撮影順に隣り合う
  写真の的中率が **1.1%**、93% の写真がどこにも繋がらなかった。
  **新しい判定手段を考えるときは必ず §4.9.2 を読むこと**（測定値・失敗の理由・
  残した資産・次の選択肢がまとまっている）。実装は PR #12。
- **画素の見え方に依存する判定は、合成データでは経路の確認しかできない。**
  §4.8 が feature print について書いた警告は、重なりの判定にもそのまま当てはまる。
  フェーズ 2.6 では「合成サンプルで分布が 2 山に割れた」「的中率 91%」を根拠に
  実写真へ一般化し、**2 度とも外した**（実測はそれぞれ「谷なし」「1.1%」）。
  合成サンプルの数字を精度の根拠にしないこと。
