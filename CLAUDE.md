...
- **グループ分けを決めるのは「実際に重なって写っているか」で、EXIF ではない**
  （設計メモ §4.9）。時刻・連番・GPS・露出・見た目は、限られた確認回数を
  **どのペアに使うか**を決めるための事前確率へ降ろしてある。したがって
  `PhotoGrouping` に新しい手がかりを足すときは、まず「それは繋ぐ判断に使うのか、
  確かめる順番に使うのか」を決めること。**繋ぐ判断は実測に一本化する**。
  ただし判定できた組が少なすぎる現場では合算スコアへ丸ごと戻る（そのことは
  診断 `overlapGroupingUnavailable` で必ず伝える）。「確認が答えを出せない現場で
  歯止めまで失うと確認前より悪くなる」は §4.6.1 から続く一貫した判断。
