...
photogrammetry-cli sort <入力フォルダ> <仕分け先フォルダ> \
    ...
    [--overlap-inliers 0.3] [--no-overlap-check] \
...
2. **見た目から「同じ場所」を見分ける**（`room-01` …）。上の手がかりはどれも
   ...
   近くても別の部屋なら引き離す**ことができます。`--no-visual` で切れます。
3. **隣り合うグループに同じ写真を重複させる**（`--overlap`、既定 15 枚）。この
   共有写真が、各グループを再構成したあとに 1 つの座標系へ合成するときの
   手がかりになります。選ぶときは視点が散らばるようにします。
4. **共有写真は「実際に重なっている」ことを確かめてから選ぶ。** 見た目の
   近さだけでは足りません — 白い壁と同じ建具ばかりの屋内では特徴の距離が
   詰まってしまい、**まったく別の場所の写真**が共有写真に紛れ込みます。
   候補を実際に位置合わせし、重なった範囲の画素が一致するかまで確かめます。
   `--overlap-inliers` で厳しさを、`--no-overlap-check` で確認そのものを
   調整できます。
5. **閾値を固定しない。** ブレ判定もグループ分けも「同じ場所」の判定も、その
...
> **`--overlap-grouping` は使わないでください（既定で切ってあります）。**
> グループ分けそのものを実際の重なりで決める試みは実装しましたが、**実データで
> 棄却しました** — 3 次元の場面を別の立ち位置から撮った 2 枚を揃える 2 次元の
> 変換は存在せず、画素の相関は重なりの有無とほとんど関係がありません（実写真
> 1424 枚で、撮影順に隣り合う写真の的中率 1.1%）。経緯と測定値は
> [設計メモ §4.9.2](docs/design-preprocess-merge.md) にあります。
