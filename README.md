...
**「見た目から同じ場所を見分ける」は入れたままにしてください**（既定で入って
います）。同じ部屋・同じ面を写した写真をまとめるので、部屋を行き来しながら
撮った写真ほど結果が良くなります。

**「確認のみ」を入れて実行すると、ファイルを作らずに診断だけ**を出します
（数分で終わります）。共有写真の不足・視点の偏り・繋がらないグループ・場所が
混ざったグループがログに出るので、**現場を離れる前に撮り直しの要否を判断
できます**。仕分けが終わったら
...
1. **グループ分けは「実際に重なって写っているか」で決める。** 2 枚を実際に
   位置合わせし、重なった範囲の画素が一致するかを測ります。これは**再構成が
   成立する条件そのもの**で、時刻や見た目のようにその推定ではありません。
   おかげで、**隣り合う部屋を続けて撮っても混ざらず、部屋を行き来しても
   分かれません**。一致度の目盛りは現場に依存しない（無関係な 2 枚は 0.04 以下、
   実際に重なる 2 枚は 0.65 以上）ので、白い壁と同じ建具ばかりの屋内でも効きます。
   `--no-overlap-grouping` で切れます（速くなりますが精度は落ちます）。
2. **EXIF は「どの組を確かめるか」に使う。** 全ペアを位置合わせすると 1424 枚で
   101 万組になるので回りません。そこで撮影時刻・GPS・露出・見た目を
   **重なりの事前確率**として使い、限られた回数を当たりやすい順に、かつ
   「答えが変わる組」から配ります（撮影順の骨格 → 切れ目の周りを厚く →
   まだ別々の塊にいる組）。**上限は `--overlap-budget`（既定は写真 1 枚あたり
   16 組）で、これがそのまま所要時間の上限**になります。使い切ったときは
   「見落としがありうる」と診断で伝えます。
3. **手がかりに依存しない。** 上の事前確率は、撮影時刻・GPS（水平誤差と測位
   時刻で足切り）・高度・方位・露出・知覚ハッシュ・サブフォルダ分けを**並列の
   証拠**として扱い、その現場で使えるものだけを重み付けして合算します。EXIF が
   失われた写真でも見た目とファイル名の連番だけで仕分きます。屋内（床下・小屋裏）
   では GPS が直前の屋外の測位のまま残るので、**位置が付いていること自体は
   信用しません**。位置合わせが 1 組も答えを出せない現場（模様の無い写真ばかり）
   では、この合算スコアだけで分ける従来の動作へ丸ごと戻ります。
4. **見た目から「同じ場所」を見分ける**（`room-01` …）。重なりは「同じ面が
   両方に写っているか」しか言えないので、**重なっていないが同じ部屋**（向かい
   合わせの壁など）は見分けられません。Vision の画像特徴によるクラスタリングが
   その層を埋め、合成のときに「どのグループが同じ部屋を写しているか」を示します。
   `--no-visual` で切れます。
5. **隣り合うグループに同じ写真を重複させる**（`--overlap`、既定 15 枚）。この
...
6. **共有写真も同じ確認を通してから選ぶ。** 見た目の
   近さだけでは足りません — 白い壁と同じ建具ばかりの屋内では特徴の距離が
   詰まってしまい、**まったく別の場所の写真**が共有写真に紛れ込みます。
   そこで候補を実際に位置合わせし、重なった範囲の画素が一致するかまで
...
7. **閾値を固定しない（重なりを除く）。** ブレ判定も「同じ場所」の判定も、その
   ...
   写真が無い現場で良品を捨てず、一続きの場所を刻みません）。**重なりだけは
   固定の既定値（0.35）を持ちます** — 目盛りが物理的に決まっているので、推定を
   挟むほうが run ごとに答えが揺れるためです。
   `--min-sharpness` / `--group-threshold` / `--visual-threshold` は逃げ道です。
   自動決定した実際の値と分布は `manifest.json` に残るので（写真は含みません）、
   結果が思わしくないときはそれを見て調整できます。
...
note=どの写真どうしが実際に重なっているかを確認しています…
note=実際に重なって写っているかで仕分けました（12320 組を位置合わせ・重なり 7104 組・重なりなし 5102 組・判定できず 114 組）。撮影時刻や GPS は「どの組を確かめるか」の順番付けにだけ使っています。
note=使った手がかり: 撮影時刻・見た目の近さ・視覚特徴の近さ・同じ場所の判定・実際の重なり（画素の一致度の閾値 0.35・指定値）
...
note=共有写真の候補 96 組を実際に位置合わせして確かめました（重なりを確認 71 組・重なっていないので除外 21 組・判定できず保留 4 組）。
