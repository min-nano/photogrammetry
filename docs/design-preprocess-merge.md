...
案(b)(c) は捨てずに残す。§10-2 のとおり、**Object Capture の実出力（高密度メッシュ +
PBR テクスチャ）が SceneKit 経由で劣化しないかはまだ未検証**で、もし劣化するなら
劣化が原理的に起きない (b) へ退避する必要があるため。変換の計算は共通なので、
書き出しだけ `--format usdz|usda` で分岐すれば両対応のコストはほぼ増えない。

なお Vectorworks 上では取り込んだ全体が 1 つの**グループ**になる。各グループを
部屋ごとにレイヤ／クラス分けしたい場合、**SceneKit のノード名（`group-01` 等）が
Vectorworks 側の名前として保持されるか**が要確認。保持されるなら「配置のみ」の
利点（部屋ごとに分かれたまま扱える）がそのまま活きる。
...
2. **SceneKit 経由でメッシュ・テクスチャが劣化しないか。** ← **残る最大の未検証事項。**
   ...
   PBR テクスチャ）で同じとは限らない**。劣化するなら案(b) 外部参照へ退避する。
   既存の usdz を 1 つ SceneKit で読んで書き直し、頂点数・テクスチャ解像度・
   見た目を比較すれば分かる（写真は不要。既に生成済みのモデルがあれば足りる）。
3. **SceneKit のノード名が Vectorworks 側で保持されるか。** 保持されれば
   `group-01` 等が部屋ごとの識別子として使える（§5.5.1 末尾）。
3. **feature print の閾値に万能な既定値は無い。** 写真依存。**固定の既定値を持たず、
...
7. **誤差の連鎖。** 鎖状に長く繋ぐほど端が歪む。フェーズ 4 の全体最適化までは
   「ループ誤差を報告する」に留める。
8. **実写真は公開できない。** 閾値調整と実機検証をどう回すか。方針は次のとおり。
