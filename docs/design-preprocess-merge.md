...
> **この章のグルーピング（§4.3 以降）は作り替えを提案中。**
> 実写真での失敗を受けて、「部屋を正しく分割する」から「Object Capture が処理
> できる大きさの窓で全体を**重なりつきに覆う**」へ目的関数を置き換える提案が
> `docs/design-loose-clustering.md` にある（**窓は視覚特徴の共視グラフの上で
> 局所的に育てる。EXIF は使わない**）。実測の記録と、採らなかった案の理由も
> そちらに集めてある。**新しく実装を始めるときは、まずそちらを読むこと。**
