...
### 4.6.1 残っている課題: 共有写真に遠くの写真が混ざる（次の一手）

**状態: 未解決。フェーズ 2 の続きとして最初に取り組むところ。**

実データ（1424 枚・屋外と屋内）で、共有写真の選定を視覚特徴の距離順に変え、
「その現場の近傍距離の中央値」を超えるペアを落とすようにしても、**まだ遠くの
写真が混ざる**。`IMG_4554〜4660` の連続した塊（group-15）に `IMG_3814〜3852`
（700 枚離れた別の場所）が 7 枚残った。

原因は分かっている。**この現場では feature print の距離が狭い帯に潰れている。**
...
次の一手は §4.1 の表に最初から挙げてある**「実際の重なり」の検証**（Vision の
画像レジストレーション、`VNHomographicImageRegistrationRequest` /
`VNTranslationalImageRegistrationRequest`）。表にも「強（ただし高コスト）／候補
ペアにのみ適用」と書いてあるとおりで、共有写真の選定はまさに候補ペアが数十組
しかない場面なので、コストの心配が要らない唯一の適用先になる。

- 隣接 1 本あたり、視覚特徴で上位に来た候補ペアだけを対象にする（数十組）
- レジストレーションが成立しない（変換が求まらない・信頼度が低い）ペアは落とす
- 1 組も成立しなければ隣接を作らない（現在の `noVisualOverlap` と同じ扱い）

実装は `FeaturePrinter` と同じくラッパー層（Vision を外へ漏らさない）に置き、
判定結果は値型で返してテスト対象外にする。判断（何組必要か、落とすか）は純
ロジック側に置くこと。

暫定の回避策としては、基準を中央値より厳しくする（例: 近傍距離の 25 パーセン
タイル）方向もあるが、**分布が潰れている以上どこかで誤判定は残る**ので、
本筋はレジストレーションによる検証。

あわせて、この現場で残っているもう 1 つの症状も記録しておく。**SNS 経由で
受け取った写真（EXIF がほぼ無い別日・別機種の JPEG）が 1 つのグループに集まる。**
手がかりが視覚情報しか無いため小さな塊が量産され、`absorbSmallGroups` が
「最も結び付きの強い隣」へ必ず吸収するのが直接の原因。吸収の判断にも
「結び付きが閾値を超えているか」を課し、超えなければ `_unassigned` へ送る案が
ある（未実装。写真を大量に `_unassigned` へ送ることになるので、実データで
影響を見てから決める）。
...
| **2** | Vision feature print による視覚クラスタリングを証拠として統合 | **実装済み**（`FeaturePrint` / `RoomClustering`、`scene` / `room` の証拠、`--no-visual` / `--visual-threshold`、manifest の `rooms` / `sharedRoom`、場所の診断）。ただし**共有写真への遠い写真の混入が残っている**（§4.6.1 が続き） |
