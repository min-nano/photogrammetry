...
**関連文書**: 方針の現在地は **`docs/roadmap.md`**（本書のフェーズ 2 = 自動の
仕分けは保留になり、当面は**手作業のフォルダ分け**を前提にする）。実測は
**`docs/measurements.md`** に設計と分けて保存してある。

`docs/design-oc-only-sampling.md` は、EXIF も視覚特徴も**一切使えない**
ときに Object Capture の結果だけでどこまで行けるかの下限を定める。結論は
「無作為抽出だけで届くのは `N ≲ 1.2〜3 × M` まで」で、本書の `sort` は
**その壁を動かすための提案者**という位置づけになる（同 §9.2 / §9.3）。
`merge` の前提が変わりうる点は同 §11.1 を参照。
