...
- **3 の切り口は「撮影順に並べたときの最弱シーム」に一本化した。** 位置 p と p+1
  ...
  いなくてもどこかで必ず切れる。
- **その重みは合計ではなく「またぐエッジの平均」。** 当初は合計にしていたが、
  実データ（1424 枚）で**まったく機能していなかった**。合計は「その位置を何本の
  エッジがまたぐか」に引きずられる。候補ペアの窓（60）が余白（下限枚数 20）より
  広いため、区間の先頭から 20 枚目あたりは構造的にまたぐ本数が少なく、内容と
  無関係にそこが最小になる。症状は明確で、**63 グループ中 57 グループがちょうど
  20 枚**、グループ間の時刻差の中央値は **2 秒**だった（＝撮影が続いている
  真ん中で切っていた）。屋外と室内が同じグループに入る主因はこれ。平均にすると
  端は「近くて強いエッジばかり」なので高く出て選ばれず、結び付きが実際に弱く
  なる場所が選ばれる。谷が緩やかなときほど差が出る（屋外→室内は時刻も位置も
  連続していて、変わるのは見た目と露出だけなので、まさに緩やかな谷になる）。
