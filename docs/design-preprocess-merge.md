...
**決め方**: 取り込み先は Vectorworks（`.usd` / `.usda` / `.usdc` / `.usdz` の読み込みに
対応）。検証用サンプルを `scripts/make-usd-samples.sh` で生成済みなので、実際に
取り込んだ結果で決める。

- `scene_flattened.usdz` / `scene_reloaded.usdz` が正しく見える → **(a)**
- `scene_referenced_usdz.usda`（または `_usda` 版）が解決される → **(b)** も選べる
- どちらも駄目 → **(c)**

(a) と (b) は排他ではない。`--format usdz|usda` で選べるようにしておくのが安全で、
実装コストもほぼ増えない（変換の計算は共通で、書き出しだけ分岐する）。
...
1. **出力形態を (a) 埋め込み / (b) 外部参照 / (c) 変換行列のどれにするか。** §5.5。
   生成済みのサンプルを Vectorworks へ取り込んだ結果で決まる。**設計上ここだけが
   まだ開いている。**
2. **SceneKit 経由でメッシュ・テクスチャが劣化しないか。** (a) を採る場合のみ問題になる。
   立方体のサンプルでは通ったが、**Object Capture の高密度メッシュと PBR テクスチャで
   同じとは限らない**。実データで要確認。
