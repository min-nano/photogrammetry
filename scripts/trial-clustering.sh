...
subject=scene
drop_blurriest=10
# 1 回の上限。**打ち切りは失敗より高くつく。** 時間切れは「あと少しで終わる
# 再構成を捨てる」うえに、その窓を落ちた扱いにして芯まで投げ直させ、しかも
# 投げた窓として記録されるので二度と試されない — 試行の結論そのものを汚す。
# 実測で 153 枚の窓が位置合わせだけに 649 秒かかっており（設計 §6.2.1）、実データの
