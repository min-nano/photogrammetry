...
struct PhotogrammetryCLI
{
	static let usage = """
		...
		sort — 大量の写真をグループへ仕分ける:
		  ...
		  グループ分けは**実際にどの写真どうしが重なって写っているか**で決める。
		  撮影時刻・GPS・露出・見た目は「どの組を確かめるか」の順番付けに使う。
		  隣り合う部屋を続けて撮っても混ざらず、部屋を行き来しても分かれない。
		  共有写真の候補も同じ確認を通してから採る。
		      ...
		      --overlap-agreement <値>    重なっていると認める画素の一致度 0.0〜1.0
		                                  （既定: 0.35。上げるほど共有写真は減るが確かになる）
		      --overlap-budget <n>        重なりを確かめる回数の上限
		                                  （既定: 写真 1 枚あたり 16 組。上げるほど
		                                  繋がりを見落としにくいが時間がかかる）
		      --no-overlap-grouping       グループ分けに重なりを使わず、撮影時刻や
		                                  見た目の合算で分ける（速いが精度は落ちる）
		      --no-overlap-check          重なりの確認を一切しない（最も速いが、
		                                  別の場所の写真が混ざりうる）
